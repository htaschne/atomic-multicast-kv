...
% Upload docs/paper/paper.tex and the files under docs/paper/figures/.
...
\foreach \x/\p in {2.05/$P_0$,4.75/$P_1$,7.20/$P_2$} {
    \node[label] at ($(orig.north west)+(\x,-0.52)$) {\p};
    \draw[lifeline] ($(orig.north west)+(\x,-0.73)$) -- ($(orig.north west)+(\x,-3.08)$);
}
\node[label, anchor=west] at ($(orig.north west)+(0.28,-0.98)$) {START $m$};
\draw[msg] ($(orig.north west)+(1.55,-0.98)$) -- ($(orig.north west)+(2.05,-0.98)$);
\draw[msg] ($(orig.north west)+(1.55,-0.98)$) -- ($(orig.north west)+(4.75,-0.98)$)
    node[midway, above, label] {$\{0,1\}$};
\node[event, anchor=west] (omdel) at ($(orig.north west)+(4.88,-1.48)$) {deliver $m$};
\node[label, anchor=west] at ($(orig.north west)+(0.28,-1.92)$) {START $m'$};
\draw[msg] ($(orig.north west)+(1.55,-1.92)$) -- ($(orig.north west)+(2.05,-1.92)$);
\draw[msg] ($(orig.north west)+(1.55,-1.92)$) -- ($(orig.north west)+(7.20,-1.92)$)
    node[midway, above, label] {$\{0,2\}$};
\node[event, anchor=west] (ompdel) at ($(orig.north west)+(2.18,-2.45)$) {deliver $m'$ before $m$};
\node[event, minimum width=0.80cm] (om) at ($(orig.north west)+(2.60,-3.34)$) {$m$};
\node[event, minimum width=0.80cm] (omp) at ($(orig.north west)+(5.25,-3.34)$) {$m'$};
...
\node[label, font=\scriptsize\bfseries] at ($(orig.north west)+(6.90,-3.34)$) {cycle};
...
\node[label, anchor=west] at ($(ack.north west)+(0.28,-0.98)$) {FINAL\_TS $m$};
\draw[msg] ($(ack.north west)+(1.55,-0.98)$) -- ($(ack.north west)+(2.05,-0.98)$);
\draw[msg] ($(ack.north west)+(1.55,-0.98)$) -- ($(ack.north west)+(4.75,-0.98)$)
    node[midway, above, label] {$\{0,1\}$};
\draw[msg, bend left=10] ($(ack.north west)+(2.05,-1.45)$) to node[above, label] {ACK} ($(ack.north west)+(4.75,-1.45)$);
\draw[msg, bend left=10] ($(ack.north west)+(4.75,-1.70)$) to ($(ack.north west)+(2.05,-1.70)$);
\node[event] (barrier) at ($(ack.north west)+(3.40,-2.22)$) {ACK barrier};
\node[event, anchor=west] at ($(ack.north west)+(3.98,-2.62)$) {deliver $m$};
\node[label, anchor=west] at ($(ack.north west)+(5.25,-2.62)$) {no overtake};
\node[event, minimum width=0.80cm] (am) at ($(ack.north west)+(2.60,-3.34)$) {$m$};
\node[event, minimum width=0.80cm] (amp) at ($(ack.north west)+(5.25,-3.34)$) {$m'$};
\draw[dep] (am) -- node[above, label] {before} (amp);
\node[label, font=\scriptsize\bfseries] at ($(ack.north west)+(6.90,-3.34)$) {no cycle};
\end{tikzpicture}
...
system,'' \emph{Commun. ACM}, vol.~21, no.~7, pp. 558--565, 1978.
...
problems,'' in \emph{Distributed Systems}, 2nd~ed., S.~Mullender, Ed.
ACM Press/Addison-Wesley, 1993, pp. 97--145.
